...
const teamList = await fetch(
  ...
  .then((res) => res.json())
  .then((data) =>
    data.map((teamSelected) => {
      const { id, name } = teamSelected;
      return { id, name };
    })
  );
export default PlayersTwelve = () => {
  ...
  useEffect(() => {
    consumirApi(teamName);
  }, [teamName]);

  const consumirApi = async (teamId) => {
    setLoading(true);
    try {
      const res = await fetch(
        `https://kings-league-api.bryan-armando.workers.dev/teams/${teamId}/players-12`
      );
